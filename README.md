...
Configure o arquivo .env na raiz do projeto (crie um novo, não versione o seu):
powershell
...
Se DATABASE_URL não for definida, o sistema usa SQLite automaticamente (data/cpp.db, criado na primeira execução).
...
Observações
Por padrão, o projeto usa SQLite para facilitar testes locais. Para PostgreSQL, configure a variável DATABASE_URL no arquivo .env.
